...
export default function ServiceDetail() {
  ...
  useEffect(() => {
    ...
    const endpoints = [`${DEFAULT_HOST}/api/services/${encodeURIComponent(id)}`,];
    ...
    async function tryFetch() {
      ...
      if (!mounted) return;
      console.warn("All endpoints failed, falling back to local servicesData. Last error:", lastError,);
      const local = servicesData && servicesData.find((s) => String(s.id) === String(id));
  ...
  const handleSubmit = async (e) => {
    ...
    if (missing.length > 0) {
      setSubmitError(`${missing.join(", ")} ${missing.length > 1 ? "are" : "is"} required`,);
      return;
    }
  ...
  // if there is no service found after loading
  if (!service) {
    return (
      <div className={serviceDetailStyles.loadingContainer}>
        <div className={serviceDetailStyles.loadingCard}>
        ...
        <div className={serviceDetailStyles.leftColumn}>
          ...
          <div className={serviceDetailStyles.detailsContainer}>
            <h3 className={serviceDetailStyles.detailsTitle}>
              <Phone size={20} /> Your Details
            </h3>

            <div className={serviceDetailStyles.detailsGrid}>
              <input required type="text" placeholder="Full Name *" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className={serviceDetailStyles.input} />

              <input type="text" required placeholder="Mobile (10 digits) *" maxLength={10} value={mobile} onChange={(e) => setMobile(e.target.value.replace(/\D/g, ""))}
                className={
              ...
              <input type="number" required placeholder="Age *" value={age} onChange={(e) => setAge(e.target.value)} className={serviceDetailStyles.input} />

              <select value={gender} required onChange={(e) => setGender(e.target.value)} className={serviceDetailStyles.input}>
              ...
              <input type="email" placeholder="Email (optional)" value={email} onChange={(e) => setEmail(e.target.value)} className={serviceDetailStyles.emailInput} />
            </div>
            ...
            <div className={serviceDetailStyles.dateSection}>
              ...
              <div className={serviceDetailStyles.paymentOptions}>
                <label className={serviceDetailStyles.paymentOption(paymentMethod === "Cash",)}>
                  <input type="radio" name="payment" value="Cash" checked={paymentMethod === "Cash"} onChange={() => setPaymentMethod("Cash")} className={serviceDetailStyles.paymentInput}/>
                  Cash
                </label>
                <label className={serviceDetailStyles.paymentOption(paymentMethod === "Online",)}>
                  <input type="radio" name="payment" value="Online" checked={paymentMethod === "Online"} onChange={() => setPaymentMethod("Online")} className={serviceDetailStyles.paymentInput} />
                  Online
                </label>
              </div>
            </div>
          ...
          <div>
            ...
            <button disabled={!isFormValid() || submitting} onClick={handleSubmit}
