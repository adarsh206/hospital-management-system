...
import { Calendar, CheckCircle, XCircle, Users, Phone, BadgeIndianRupee, } from "lucide-react";
...
function normalizeAppointment(a) {
  ...
  const doctorName = (a.doctorId && typeof a.doctorId === "object" && a.doctorId.name) || a.doctorName || a.doctor || "Doctor";

  const doctorImage = (a.doctorId && typeof a.doctorId === "object" && a.doctorId.imageUrl) || a.doctorImage || a.doctorImageUrl || "";
...
export default function DashboardPage({ apiBase }) {
  ...
  async function fetchAppointments() {
    ...
    setError(null);
    try {
      const basePath = `${API}/api/appointments/doctor/${encodeURIComponent(doctorId,)}`;
      const url = `${basePath}`;
      console.log(url);
  ...
  return (
    <div className={dashboardStyles.statCard}>
      <div className={dashboardStyles.statContent}>
        ...
        <div className={`${dashboardStyles.statIconContainer} ${accentTop} ${accentBottom}`}>
    ...
    <div className="w-full">
      {!editing ? (
        ...
        <div className={dashboardStyles.rescheduleForm}>
          <input type="date" value={date} min={minDate} onChange={(e) => setDate(e.target.value)} className={dashboardStyles.rescheduleDateInput} />
          <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className={dashboardStyles.rescheduleTimeInput} />
          <div className={dashboardStyles.rescheduleButtons}>
            <button onClick={save} className={dashboardStyles.saveButton}>Save</button>
            <button onClick={cancel} className={dashboardStyles.cancelButton}>Cancel</button>
          </div>
        </div>
      )}
    </div>
