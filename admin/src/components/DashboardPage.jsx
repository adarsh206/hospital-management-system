import { BadgeIndianRupee, CalendarRange, Search, User, UserRoundCheck, XCircle } from "lucide-react";
    ...
    <div className={s.pageContainer}>
        <div className={s.maxWidthContainer}>
            ...
            <div className={s.statsGrid}>
                <StatCard icon={<User className="w-6 h-6" />} label="Total Doctors" value={totals.totalDoctors}/>
                {/** shows count fetch from the backend */}
                <StatCard icon={<UserRoundCheck className="w-6 h-6" />} label="Total Registered Users" value={patientCountLoading ? "Loading..." : (patientCount ?? totals.totalLoginPatients)}/>
                <StatCard icon={<CalendarRange className="w-6 h-6" />} label="Total Appointments" value={totals.totalAppointments}/>
                <StatCard icon={<BadgeIndianRupee className="w-6 h-6" />} label="Total Earnings" value={`₹ ${totals.totalEarnings.toLocaleString()}`}/>
                <StatCard icon={<BadgeIndianRupee className="w-6 h-6" />} label="Total Earnings" value={`₹ ${totals.totalEarnings.toLocaleString()}`}/>
            ...
            <div className="mb-6">
                ...
                <div className={s.searchContainer}>
                    ...
                    <button className={s.clearButton + " " + s.cursorPointer} onClick={() => {
                        setQuery("");
                        setShowAll(false);
                    }}>Clear</button>
                </div>
            ...
            <div className={s.tableContainer}>
              <div className={s.tableHeader}>
                ...
                <p className={s.tableCount}>{loading ? "Loading..." : `Showing ${visibleDoctors.length} of ${filteredDoctors.length}`}</p>
              </div>
              {error && (
              ...
              <div className={s.tableWrapper}>
                <table className={s.table}>
                    ...
                    <tbody className={s.tableBody}>
                        ...
                        <tr key={d.id} className={s.tableRow + " " + (idx % 2 === 0 ? s.tableRowEven : s.tableRowOdd)}>
                            ...
                            <td className={s.tableCell + " " + s.doctorSpecialization}>{d.specialization}</td>
                            <td className={s.tableCell + " " + s.feeText}>₹ {d.fee}</td>
                            <td className={s.tableCell + " " + s.appointmentsText}>{d.appointments.total}</td>
                            <td className={s.tableCell + " " + s.completedText}>{d.appointments.completed}</td>
                            <td className={s.tableCell + " " + s.canceledText}>{d.appointments.canceled}</td>
                            <td className={s.tableCell + " " + s.earningsText}>₹ {d.earnings.toLocaleString()}</td>
                        </tr>
                        ))}
                    </tbody>
                </table>
              ...
              </div>
              {
                filteredDoctors.length > INITIAL_COUNT && (
                    <div className={s.showMoreContainer}>
                        <button className={s.showMoreButton + " " + s.cursorPointer} onClick={() => setShowAll((s) => !s)}>
