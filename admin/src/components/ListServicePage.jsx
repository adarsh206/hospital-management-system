...
import { Image as ImageIcon, Edit2, Trash2, Check, X, ChevronDown, Search, Calendar, Plus } from "lucide-react";
...
export default function ListServicePage() {
  ...
  function addToast(message, type = "success", ttl = 3000, position = "bottom-right", animated = false) {
...
function sortSlotsForDisplay(slots = []) {
  ...
  // to fetch services from the server
  ...
  // convert the array of slots into slot object
  ...
  // gives output wit date and time like DD-MM-YYY with HH: MM AM/PM
  ...
  async function startEdit(service) {
    ...
    const normalized = {
      ...
      available: latest.available ?? latest.availability === "Available" ?? true,
  ...
  // format the slots for backend ex: DD-MM-YYYY with HH:MM AM/PM
  ...
  // to edit any services we save then for a particular service
