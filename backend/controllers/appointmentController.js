...
export const createAppointment = async (req, res) => {
    try {
        ...
        if(existingBooking) {
            return res.status(409).json({ success: false, message: "You already have an appointment booked with this doctor at the same date and time." });
        }
        ...
        try{
            ...
            console.warn("Doctor lookup failed:", err?.message || err);
        }
